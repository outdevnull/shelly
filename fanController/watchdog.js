...
function gkrs(cb) {
  Shelly.call("KVS.GetMany", {}, function(r, e) {
    if (e || !r || !r.items) { lg("WARN", "kvs getmany skip"); cb(); return; }
    let items = r.items;
    let ks = Object.keys(items);
    let kd = {};
    for (let i = 0; i < ks.length; i++) {
      let k = ks[i];
      // Exclude runtime-state keys: s.<id>.ok and s.<id>.fails
      if (k.slice(0, 2) === "s." && (k.slice(-3) === ".ok" || k.slice(-6) === ".fails")) continue;
      kd[k] = items[k].value;
    }
    let kks = Object.keys(kd);
    let ln = ["// kvs_restore auto-generated by watchdog", "let keys = {"];
    for (let i = 0; i < kks.length; i++) {
      let k = kks[i];
      let v = String(kd[k]).split("\\").join("\\\\").split('"').join('\\"').split("\n").join("\\n");
      ln.push('  "' + k + '": "' + v + '"' + (i < kks.length - 1 ? "," : ""));
    }
    ln.push("};");
    ln.push("let ks = Object.keys(keys);");
    ln.push("let i = 0;");
    ln.push("function nx() {");
    ln.push("  if (i >= ks.length) { print(\"kvs_restore complete\"); Shelly.call(\"Script.Stop\", { id: Shelly.getCurrentScriptId() }, null); return; }");
    ln.push("  let k = ks[i]; i++;");
    ln.push("  Shelly.call(\"KVS.Get\", { key: k }, function(r, e) {");
    ln.push("    if (!e && r) { nx(); return; }");
    ln.push("    Shelly.call(\"KVS.Set\", { key: k, value: keys[k] }, function() { nx(); });");
    ln.push("  });");
    ln.push("}");
    ln.push("nx();");
    let code = ln.join("\n"); ln = null; kd = null;
